...
public class FFmpegUtil {
    ...
    public static String[] extractAudio(String inputPath, String outputPath) {
        //-vn: disable video
        String extractAudioCmd = "ffmpeg -i %s -f mp3 -vn %s";
     ...
     * 压缩视频
     */
    public static String[] compressVideo(String inputPath, String outputPath) {
...
//        String s = "ffmpeg -y -i %s -strict -2 -vcodec libx264 -preset superfast -crf 30 -acodec copy -ar 44100 -ac 2 -b:a 96k -vf scale=-2:960 %s";
////        String s = "ffmpeg -i %s -vcodec libx264 -crf 30 -preset superfast -y -acodec libmp3lame %s";
//        s = String.format(s, inputPath, outputPath);
//        return s.split(" ");
    }
     ...
     * 按宽高比裁剪视频
     */
    public static String[] cropVideo(@NotNull String inputPath, @NotNull String outputPath, int width, int height, boolean landscape) {
        if (!landscape) {
            // 减高度
            return new String[]{
                    "ffmpeg", "-y",
                    "-i", inputPath,
                    "-vf", "crop=iw:" + height + "*iw/" + width,
                    outputPath
            };
        } else {
            // 减宽度
...
//    }
     ...
     *
     * @param inputPath  aac or mp3
     * @param outputPath pcm16
     * @return decode audio success or not
     */
    public static String[] decodeAudio2PCM16(String inputPath, String outputPath) {
        String decodeAudioCmd = "ffmpeg -i %s -f s16le -ar 16000 %s";
//        String decodeAudioCmd = "ffmpeg -i %s -f s16le -ar 16000 -acodec pcm_s16le %s";
        decodeAudioCmd = String.format(decodeAudioCmd, inputPath, outputPath);
        return decodeAudioCmd.split(" ");
    }
