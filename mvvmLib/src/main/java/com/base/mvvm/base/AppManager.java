...
public class AppManager {
    ...
    public void finishActivity(List<Class> cls) {
        for (Class activity : cls) {
            finishActivity(activity);
        }
    }
    ...
    /**
     * 获取指定的Activity(不包含自己本身)
     * @param current 当前
     * @return 非当前
     */
    @Nullable
    public Activity getOtherActivity(@Nullable  Activity current) {
        if (activityStack != null && current !=null)
            for (Activity activity : activityStack) {
                if (activity == current){
                    continue;
                }
                if (activity.getClass().equals(current.getClass())) {
                    return activity;
                }
            }
        return null;
...
//            其实android的机制决定了用户无法完全退出应用，当你的application最长时间没有被用过的时候，android自身会决定将application关闭了。
    ...
    public void AppExitExcludeActivity(Class<?> cls) {
        try {
            finishAllExcludeActivity(cls);
        } catch (Exception e) {
            if (activityStack!=null){
                activityStack.clear();
            }
     ...
     * 结束 直到指定 activity
     */
    public boolean endUntilTarget(Class<?> cls) {
        if (activityStack == null || activityStack.isEmpty()) {
            return false;
        }
        int index = -1;
