package com.base.mvvm.utils.compression;
...
public class Luban {
    ...
    public Luban launch() {
        Checker.checkNotNull(mFile, "the image file cannot be null, please call .load() before this method!");
    ...
    public Luban setCompressPath(String path) {
        mThumbPath = path;
        return this;
    }
    ...
    private File thirdCompress(@NonNull File file) throws IOException {
        ...
        String thumb = mThumbPath;
        if (thumb.isEmpty()) {
            thumb = mCacheDir.getAbsolutePath() + File.separator + fileName.substring(0, fileName.indexOf(".")) + "_compress.jpg";
        }

        File outFile = new File(thumb);
        Engine engine = new Engine(file.getAbsolutePath(), outFile, false);
    ...
    private File saveImage(String filePath, Bitmap bitmap, long size) {
        Checker.checkNotNull(bitmap, TAG + "bitmap cannot be null");
