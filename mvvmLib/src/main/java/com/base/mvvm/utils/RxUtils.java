...
import com.base.mvvm.http.BaseResponse;
import com.base.mvvm.http.ExceptionHandle;
...
public class RxUtils {
    ...
    private static class HandleFuc<T> implements Function<BaseResponse<T>, T> {
        ...
        public T apply(BaseResponse<T> response) {
            if (!response.isOk())
                throw new RuntimeException(!"".equals(response.getStatus() + "" + response.getMsg()) ? response.getMsg() : "");
