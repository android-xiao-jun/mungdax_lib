...
public class KLog {
     ...
     * 记录到本地文件，，后面上传
     */
    public static void log(@Nullable String tag, @Nullable Object msg) {
        logWrite(tag, msg, 0);
    }
     ...
     * 记录到本地文件，，后面上传
     */
    public static void log(@Nullable Object msg) {
        log(null, msg);
    }

    private static void logWrite(@Nullable String tagStr, @Nullable Object objectMsg, int i) {
        if (!IS_SHOW_LOG) {
            return;
        }
        String msg;
        ...
        int diff = 0;
        if ("launch".equals(objectMsg) || "launchLoading".equals(objectMsg)) {
            diff = 2;
        }

        int index = 4 + i + diff;

        String className = stackTrace[index + 1].getFileName();
        ...
        if (objectMsg instanceof Throwable) {
            msg = "";
            StackTraceElement[] stackTraceElements = ((Throwable) objectMsg).getStackTrace();
            stringBuilder.append(((Throwable) objectMsg).getMessage());
            stringBuilder.append(Arrays.toString(stackTraceElements));
        } else if (objectMsg == null) {
        ...
        LogWriteManager.logWrite(tag, logStr, i == 0);
    }
