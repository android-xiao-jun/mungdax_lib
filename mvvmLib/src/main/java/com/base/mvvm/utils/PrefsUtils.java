...
public final class PrefsUtils {
    ...
    private static final Map<String, PrefsUtils> sSPMap = new HashMap<>();
    private final SharedPreferences sp;

    /**
     * 获取SP实例
     *
     * @return {@link PrefsUtils}
     */
    public static PrefsUtils get() {
        return get("");
    }
     ...
     * @return {@link PrefsUtils}
     */
    public static PrefsUtils get(String spName) {
        if (isSpace(spName)) spName = "PrefsUtils";
        PrefsUtils sp = sSPMap.get(spName);
        if (sp == null) {
            sp = new PrefsUtils(spName);
            sSPMap.put(spName, sp);
        }
        return sp;
    }

    private PrefsUtils(final String spName) {
